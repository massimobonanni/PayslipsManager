Yes — and one small correction first: for **Azure Developer CLI**, the root config file is **`azure.yaml`**, not `azd.yaml`. In a typical `azd` template, the repo includes an `infra` folder for Bicep/Terraform, a `src` folder for application code, and an `azure.yaml` file that maps services to Azure hosts such as **App Service** and **Function**. ([Microsoft Learn][1])

Below is the **reworked Copilot prompt**, split into simple tasks and with the **test part removed**.
